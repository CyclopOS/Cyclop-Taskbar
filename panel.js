/*
 * This file is part of the Cyclop Taskbar extension for Gnome 3
 *
...
var dtpPanel = Utils.defineClass({
    ...
    enable : function() {
        ...
        panelBoxes.forEach(b => {
            this[b].allocate = (box, flags, isFromCyclopTaskbar) => {
                if (isFromCyclopTaskbar) {
        ...
        this.panel.actor.add_style_class_name('CyclopTaskbarMainPanel ' + this.getOrientation());
    ...
    disable: function () {
        ...
        if (!this.isStandalone) {
            ...
            ['vertical', 'horizontal', 'CyclopTaskbarMainPanel'].forEach(c => this.panel.actor.remove_style_class_name(c));
    ...
    _setPanelPosition: function() {
        ...
        Object.keys(St.Side).forEach(p => {
            let cssName = 'cycloptaskbar' + p.charAt(0) + p.slice(1).toLowerCase();
...
var dtpSecondaryPanel = Utils.defineClass({
    Name: 'CyclopTaskbar-SecondaryPanel',
    Extends: St.Widget,
...
var dtpSecondaryAggregateMenu = Utils.defineClass({
    Name: 'CyclopTaskbar-SecondaryAggregateMenu',
