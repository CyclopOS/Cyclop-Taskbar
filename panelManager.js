...
var dtpPanelManager = Utils.defineClass({
    Name: 'CyclopTaskbar.PanelManager',
    ...
    enable: function(reset) {
        ...
        global.CyclopTaskbar.panels = this.allPanels;
        global.CyclopTaskbar.emit('panels-created');
    ...
    _getBoxPointerPreferredHeight: function(boxPointer, alloc, monitor) {
        if (boxPointer._dtpInPanel && boxPointer.sourceActor && Me.settings.get_boolean('intellihide')) {
            ...
            let panel = Utils.find(global.CyclopTaskbar.panels, p => p.monitor == monitor);
...
var IconAnimator = Utils.defineClass({
    Name: 'CyclopTaskbar.IconAnimator',
...
function newUpdateHotCorners() {
    ...
    for (let i = 0; i < this.monitors.length; i++) {
        let panel = Utils.find(global.CyclopTaskbar.panels, p => p.monitor.index == i);
...
function _newLookingGlassResize() {
    let primaryMonitorPanel = Utils.find(global.CyclopTaskbar.panels, p => p.monitor == Main.layoutManager.primaryMonitor);
